...
//   // imgslide.style.transform = `translateX(calc(${(index - 1) * -100}% - ${(index - 1) * 2}rem))`;
...
const activePortfolio = () => {
  const imgslide = document.querySelector('.portfolio-carousel .img-slide');
  const portfolioDetails = document.querySelectorAll('.portfolio-detail');
  const totalSlides = 7; // total number of slides (indexes 0–6)

  // Wrap index so it loops infinitely
...
document.querySelector('.contact-box form').addEventListener('submit', function (e) {
  ...
  fetch('https://formsubmit.co/ajax/yashgautam2407@gmail.com', {
    method: 'POST',
    body: data
  })
